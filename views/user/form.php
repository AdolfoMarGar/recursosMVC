...
// VISTA PARA INSERCIÓN/EDICIÓN DE USERS

extract($data);   // Extrae el contenido de $data y lo convierte en variables individuales  ej ($user)

// Vamos a usar la misma vista para insertar y modificar. Para saber si hacemos una cosa u otra,
// usaremos la variable $user: si existe, es porque estamos modificando un user. Si no, estamos insertando uno nuevo.
if (isset($user)) {   
...
// Sacamos los datos del user (si existe) a variables individuales para mostrarlo en los inputs del formulario.
// (Si no hay user, dejamos los campos en blanco y el formulario servirá para inserción).
$id = $user->id ?? ""; 
...
// Creamos el formulario con los campos del user
